...
def get_reservation_by_id(id_reservation: str, db: Session = Depends(get_db)):
    reservation = db.query(models.Reservation).filter(models.Reservation.id_reservation == id_reservation).first()
    if not reservation:
...
def update_reservation(id_reservation: str, request: schemas.ReservationCreate, db: Session = Depends(get_db)):
    reservation = db.query(models.Reservation).filter(models.Reservation.id_reservation == id_reservation).first()
    if not reservation:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Reservation introuvable")
    ...
    updated_reservation = request.model_dump(exclude={"id_reservation", "id_vehicule", "id_client", "id_user"})

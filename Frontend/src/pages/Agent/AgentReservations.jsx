...
function AgentReservations() {
  ...
  const reservationsFiltrees = filter === 'tous'
    ? reservations
    ...
    <div className='bg-gray-100 min-h-screen'>
      ...
      <div className='p-5 flex gap-2 flex-wrap'>
        <button
          ...
          className={`px-4 py-2 rounded font-medium ${filter === 'tous' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
        >
        ...
        {Object.keys(statusLabels).map(s => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded font-medium ${filter === s ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
          >
              ...
              <tbody>
                {reservationsFiltrees.map((res) => (
                  <tr key={res.id_reservation} className='border-b hover:bg-gray-50'>
                    <td className='px-4 py-3'>{res.client?.nom || 'N/A'}</td>
                    ...
                    <td className='px-4 py-3'>
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusStyle(res.status)}`}>
